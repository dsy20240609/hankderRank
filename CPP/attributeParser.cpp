...
queries follow. 
Each query consists of string that references an attribute in the source program.More formally, 
each query is of the form ~ where  and  are valid tags in the input.
...
int main() {
    ...
    int numberOfInput;
    int numberOfQuery;
    cin>>numberOfInput>>numberOfQuery;
